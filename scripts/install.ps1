...
if ($detected | Where-Object { $_.Label -eq 'Codex' }) {
  ...
  Write-Host ''
  Write-Host '  [windows]'
  Write-Host '  sandbox = "elevated"'
  Write-Host ''
  Write-Host '  [sandbox_workspace_write]'
  Write-Host '  network_access = true'
  Write-Host ("  writable_roots = [""{0}""]" -f $storeToml)
  Write-Host ''
  Write-Host 'On Windows the credential folder must exist before Codex can write to it:'
  Write-Host ("  New-Item -ItemType Directory -Force '{0}'" -f $store)
  Write-Host ''
  Write-Host '"elevated" is the mode that really blocks the network when you do not allow it.'
  Write-Host 'Skip all this if your config already sets sandbox_mode to danger-full-access.'
