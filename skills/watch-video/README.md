...
bash scripts/setup.sh --with-whisper   # add local speech-to-text
```
...
| `whisper` | optional | Transcripts when a source ships no captions |
...
| `--whisper` | off | Transcribe locally if there are no captions |
...
watch.sh "https://soundcloud.com/x/y" --whisper

# an Arabic video with no captions, transcribed locally
watch.sh ./ads/cut-03.mp4 --lang "ar.*" --whisper --outdir ./analysis/cut-03
```
...
3. If there are no captions and `--whisper` was passed, a local speech-to-text engine produces the transcript.
...
- `--whisper` is accurate but much slower than downloading a caption file, which is why it is opt-in.
