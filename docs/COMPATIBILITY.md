...
| Amp 0.0.1789934445 | 7 of 7 in `amp skills list`. Its own 12 built-in skills were the only ones in the control | `amp skills add itqanlab/agent-toolkit/plugins/<name>/skills/<name>`: installed `watch-video` from GitHub. The whole-repo command `amp skills add itqanlab/agent-toolkit` needs the copy in `.agents/skills/`. See below |
...
**Amp finds skills in `skills/`, `.agents/skills/`, `.claude/skills/`, the repository root and one level below it, and nowhere else.** Tested with small layouts. It does not look in `plugins/<name>/skills/<name>`, or three folders down. It also skips symbolic links: a link in `skills/` and a relative link both gave `No SKILL.md files found`. Only a real copy is found. Our old `skills/<name>` layout was found, so moving to the plugin layout broke the whole-repository command `amp skills add itqanlab/agent-toolkit`. The fix is a generated copy of each skill folder in `.agents/skills/<name>/`, written and checked byte for byte by `npm run catalog`. With the copy, `amp skills add <path to the repository>` installed all 7 skills, and `watch-video` was identical to the source. `npx skills add <repository> --list` still lists each skill once, and Codex still installs and shows all 7. A whole-repository Gemini install found nothing, with or without the copy, so it needs `--path` as before. Copilot was not run again against the copy. Its plugin route reads `.claude-plugin/marketplace.json`, which the copy does not touch. The Amp test used a local path to the repository. The `itqanlab/agent-toolkit` form on GitHub was not run until the copy was pushed. We keep the plugin layout, because Codex requires it.
