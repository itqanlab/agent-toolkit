...
- `description` — up to 1024 chars. This is the only thing an agent sees when deciding whether to invoke the skill, so it must carry the trigger phrases a user would actually type. Write it in this order: one short sentence saying what the skill does, then `Triggers: 'a', 'b', 'c'.`, then the detail such as what it needs and where it works. The phrases go first because Codex shortens a long skill list from the end. At 40 installed skills each description is cut to about 430 characters, and at 80 to about 190. Keep the first sentence under about 120 characters. `validate.sh` warns when the phrases start after character 200. End the phrase list with a quote and a full stop (`'.`), because that is where the parser stops reading it. **Quote the whole value** — `description: "… Triggers: 'a', 'b'. …"` — because the colon in `Triggers:` inside a bare scalar is invalid YAML and the upstream validator rejects it. Use single quotes for the phrases so the outer double quotes stay valid. The generated site leads each skill page with these phrases, so a skill without them loses its most useful section.
...
2. Write `plugins/<name>/skills/<name>/SKILL.md`. The frontmatter `name` must match `<name>`. Under `metadata:` set `version`, `category`, `short` and `starter`. The category must be one of the ids in `catalog/categories.json`. `short` is 25 to 64 characters and shows in Codex's skill list and on the plugin card. `starter` finishes the sentence "Use $name to ...", for example `tell me why the last deploy failed`. Add `access: read` if the skill changes nothing.
