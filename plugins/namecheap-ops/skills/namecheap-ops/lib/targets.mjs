...
/**
 * Fields a target can be complete without. Namecheap needs one: the relay is
 * only for people whose own address is not the one Namecheap has whitelisted.
 */
const requiredFields = (spec) => Object.keys(spec.fields).filter((f) => !(spec.optional || []).includes(f));
...
export function listTargets(spec) {
  ...
  return [...names.values()]
    .map((entry) => ({
      ...
      complete: requiredFields(spec).every((field) => isSet(entry.fields[field])),
...
function incomplete(spec, target) {
  const missing = Object.entries(spec.fields)
    .filter(([field]) => requiredFields(spec).includes(field) && !isSet(target.fields[field]))
