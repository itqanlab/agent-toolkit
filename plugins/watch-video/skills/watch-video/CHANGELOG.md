...
- Installed as a Claude Code plugin, the slash command is now `/watch-video:watch-video`, with the plugin name in front. This has been true since 1.3.0, and this release marks it as breaking. Use the new name, or ask in words, which works as before. A copy in a skills folder keeps the plain name `/watch-video`.
