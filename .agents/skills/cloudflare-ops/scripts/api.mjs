#!/usr/bin/env node
// api.mjs — search Cloudflare's API description offline.
//
// The wrapped commands in cf.mjs cover the everyday work. For anything else the
// question is always the same: which endpoint, and what does it want in the body?
// Guessing produces confident, wrong code. This answers it from Cloudflare's own
// OpenAPI description — about 2000 endpoints — cached next to the credentials.
//
...
//
// Pair it with request() from cf.mjs, which already reaches every endpoint.
...
const SPEC_PATH = join(storeRoot(), 'cache', 'cloudflare-openapi.json');
const STALE_AFTER_DAYS = 30;

async function download() {
  console.error('Downloading the Cloudflare API description (about 22 MB, once)...');
  ...
  if (!response.ok) {
    throw new Error(
      ...
      'It needs network access. The wrapped commands in cf.mjs do not depend on this.',
    );
  }
...
/**
 * Follow a "#/components/..." pointer. The description is almost entirely
 * references — an unresolved one tells the reader nothing, which is the whole
...
function describeSchema(schema, spec, depth = 0, seen = new Set()) {
  ...
  // allOf is a merge: every part applies at once.
  if (node.allOf) return node.allOf.flatMap((s) => describeSchema(s, spec, depth, seen));

  // oneOf/anyOf is a choice. Cloudflare uses it heavily — a DNS record has one
  // variant per record type — so printing them all buries the answer. Show the
  // first in full and name the rest by whatever field distinguishes them.
  const variants = node.oneOf || node.anyOf;
  if (variants) {
    const pad = '  '.repeat(depth + 1);
    const labels = variants
      .map((v) => deref(v, spec, new Set(seen)))
      .map((v) => v?.properties?.type?.enum?.[0] || v?.properties?.type?.const || v?.title)
      .filter(Boolean);

    const head = variants.length === 1
      ? []
      : [`${pad}(${variants.length} shapes${labels.length ? `: ${labels.slice(0, 12).join(' | ')}${labels.length > 12 ? ' | ...' : ''}` : ''})`,
         `${pad}showing the first; the others differ mainly in "content"`];
...
const COMMANDS = {
  async search(words) {
    ...
    if (!terms.length) throw new Error('usage: api.mjs search <words...>   e.g. api.mjs search waf rule');
    ...
    if (!matches.length) {
      ...
      console.log('Try fewer or more general words — the paths use Cloudflare\'s own naming.');
      return;
    }
  ...
  async show([path, method = 'get']) {
    ...
    console.log(`\ncall it with:\n  import { request } from './scripts/cf.mjs';`);
