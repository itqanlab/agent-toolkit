...
- Installed as a Claude Code plugin, the slash command is now `/cloudflare-ops:cloudflare-ops`, with the plugin name in front. This has been true since 1.2.0, and this release marks it as breaking. Use the new name, or ask in words, which works as before. A copy in a skills folder keeps the plain name `/cloudflare-ops`.
